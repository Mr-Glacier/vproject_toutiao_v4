...
public class Dao_ArticleContent extends DaoFather {
    public Dao_ArticleContent(int choseTable) {
        super(choseTable);
    }


    public void Methoc_ChangeDownState(int ID,String time){
        String sql1 = "update T_ArticleContent set  DownState = '是' where ID ="+ID;
        MethodIUD(sql1);
        String sql2 =" update T_ArticleContent set DownTime = '"+time+"' where ID =" +ID;
        MethodIUD(sql2);
    }
